...
def assert_paser_valid(args):
    ...
    assert (isinstance(eval(args.width), float) or isinstance(eval(args.width), int)), "-width: should be a float/int number"
    assert (isinstance(eval(args.level), float) or isinstance(eval(args.level), int)), "-level: should be a float/int number"
...
def read_data(args):
    ...
    Vols = []
    Segs = []
    Folder_Vol = os.path.join(args.input_root, 'crop_vol')
    Folder_Seg = os.path.join(args.input_root, 'crop_msk')
    if args.Volume[0] == 'all':
        Vols = sorted(glob.glob(os.path.join(Folder_Vol, 'UC*')))
        Segs = sorted(glob.glob(os.path.join(Folder_Seg, 'UC*')))
        for _index_ in range(len(Vols)):
            ...
            if 'High' in Grade_info or 'high' in Grade_info:
                Feature_Table[ID]['Histological grade'] = 'HG'
            elif 'Low' in Grade_info or 'low' in Grade_info:
                Feature_Table[ID]['Histological grade'] = 'LG'
            else:
                Feature_Table[ID]['Histological grade'] = 'None'
            if data_Table[ID][6] == '' or data_Table[ID][6] == None:
            ...
            #print(f"Glob from {os.path.join(Vols[_index_])}")
            Feature_Table[ID]['tumor'] = glob.glob(os.path.join(Vols[_index_], '*.tif'))[0]
            Feature_Table[ID]['mask'] = glob.glob(os.path.join(Segs[_index_], '*.png'))[0]
            #Vols[_index_] = glob.glob(os.path.join(Vols[_index_], '*.tif'))[0]
            #Segs[_index_] = glob.glob(os.path.join(Segs[_index_], '*.png'))[0]
    else:
        N = len(args.Volume)
        for _index_ in range(N):
            Vol = glob.glob(os.path.join(Folder_Vol, f'{args.Volume[_index_]}*'))[0]
            Seg = glob.glob(os.path.join(Folder_Seg, f'{args.Mask[_index_]}*'))[0]
            ...
            if data_Table[ID][6] == '' or data_Table[ID][6] == None:
                ...
                Feature_Table[ID]['T stage'] = data_Table[ID][6]
            Feature_Table[ID]['Lymph-Invasion'] = data_Table[ID][9]
            Feature_Table[ID]['tumor'] = glob.glob(os.path.join(Vol, '*.tif'))[0]
            Feature_Table[ID]['mask'] = glob.glob(os.path.join(Seg, '*.png'))[0]
            #Vols.append(glob.glob(os.path.join(Vol, '*.tif'))[0])
            #Segs.append(glob.glob(os.path.join(Seg, '*.png'))[0])
...
def Extract_features(args):
    ...
    extractor.enableImageTypeByName('Wavelet', customArgs={'level':1})
    extractor.enableImageTypeByName('Square')
    extractor.enableImageTypeByName('SquareRoot')
    extractor.enableImageTypeByName('Logarithm')
    extractor.enableImageTypeByName('Exponential')
    extractor.enableImageTypeByName('Gradient', customArgs={'gradientUseSpacing':False})
    extractor.enableImageTypeByName('LBP2D', customArgs={'lbp2Dmethod':'default', 'lbp2DRadius':3, 'lbp2DSamples':36})
    ...
    for ID in Feature_Table.keys():
        ...
        features = extractor.execute(IMG, maskFilepath, 255)
        F = {}
    ...
    for _filter_ in list(F.keys()):
        ...
        for _feature_ in feature_types:
            ...
            for ID in list(Feature_Table.keys()):
                ...
                print(_filter_, _feature_, feature_value, _Max_, _Min_)
            ...
            _index_ = 0
            for ID in list(Feature_Table.keys()):
                Feature_Table[ID]['Features'][_filter_][_feature_] = offset + scale_factor * (buffer_list[_index_] - _Min_)
                _index_ += 1

def save_results(args):
    json_path = os.path.join(args.output_folder, 'Features.txt')
    json_file = open(json_path, 'w')
    json_content = json.dumps(Feature_Table, indent = 4)
    json_file.writelines(json_content)
    json_file.close()

    csv_path = os.path.join(args.output_folder, 'Features.csv')
...
def main():
    parser = argparse.ArgumentParser(
            prog='Radiomics Feature Analysis',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            description=API_description)

    parser.add_argument('-input_root', action = 'store', type = str, help = 'The absolute path to input root.')
    parser.add_argument('-Table', action = 'store', type = str, help = 'The absolute path to the DATA TABLE (*.csv).')
    parser.add_argument('-Volume', nargs = '+', help = 'ex: -Volume Vol1.tif Vol2.tif ...')
    parser.add_argument('-Mask', nargs = '+', help = 'ex: -Mask Msk1.png Msk2.png ...')
    parser.add_argument('-output_folder', action = 'store', help = 'The absolute path to the output folder used to store extracted Feature Table')
    parser.add_argument('-width', action = 'store', type = str, help = 'window width')
    parser.add_argument('-level', action = 'store', type = str, help = 'window level')
    parser.add_argument('-normalize', action = 'store', type = str, help = 'True/False')
