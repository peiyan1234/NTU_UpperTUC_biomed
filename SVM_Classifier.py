...
def splitter(Feature_Table, output_folder, train_index, test_index):
    Train_Table = {}
    Test_Table = {}
    ...
    Train_json_path = os.path.join(output_folder, 'Train_data.txt')
    Train_json_file = open(Train_json_path, 'w')
    Train_json_content = json.dumps(Train_Table, indent = 4)
    Train_json_file.writelines(Train_json_content)
    Train_json_file.close()
    
    Test_json_path = os.path.join(output_folder, 'Test_data.txt')
    Test_json_file = open(Test_json_path, 'w')
    Test_json_content = json.dumps(Test_Table, indent = 4)
...
def get_Data(SelectionResult, N_features, json_path, LabelName):
...
def backward_selection(SelectionResult, N_features, Train_json_path, Test_json_path, Label, Feature_ratio, K, select_feature_counter):
    N_samples, X_train, y_train, select_map = get_Data(SelectionResult, N_features, Train_json_path, Label)
    #rng = random.randint(0, N_samples)
    C = 8
    tol = 1e-3
    nu = 0.15
    #clf = SVC(gamma='scale', class_weight='balanced', C=C, tol=tol)
    clf = NuSVC(class_weight='balanced', tol=tol, nu=nu)
    ...
    skf = RepeatedStratifiedKFold(n_splits=K, n_repeats=30, random_state=0)
        ...
        for _index_ in range(N_features):
            if _index_ in F:
                ...
                for train, test in skf.split(X_train, y_train):
                    clf.fit(X_tmp[train], y_train[train])
                    y_predict = clf.predict(X_tmp[test])
                    acc_tmp = accuracy_score(y_train[test], y_predict)
                    acc += acc_tmp
                acc = float(acc) / (skf.get_n_splits(X=X_train, y=y_train))
                F.append(_index_)
                if acc > max_acc:
                    max_acc = acc
                    idx = _index_
    ...
    _, X_test, y_test,_ = get_Data(SelectionResult, N_features, Test_json_path, Label)
    ...
    sol = NuSVC(class_weight='balanced', tol=tol, nu=nu)
    ...
    sol.fit(X_selected, y_train)
    y_inference = sol.predict(X_test[:, F])
    print(f"Predict: {y_inference}")
    y_score = sol.fit(X_selected, y_train).decision_function(X_test[:, F])
    
    return accuracy_score(y_test, y_inference), X_test[:, F], y_test, y_inference, y_score, sol, select_feature_counter

def forward_selection(SelectionResult, N_features, Train_json_path, Test_json_path, Label):
    forward_result = {}
...
def get_args():
    parser = argparse.ArgumentParser(
            prog='Radiomics SVM Classifier',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            description=API_description)
    
    parser.add_argument('-Table', action = 'store', type = str, help = 'The absolute path to the Feature table (*.txt).')
    parser.add_argument('-LeavePout', action = 'store', help = 'an integer, P for leave-P-out cross validation')
    parser.add_argument('-output_folder', action = 'store', help = 'The absolute path to the output folder used to store splitted data')
    parser.add_argument('-method', action = 'store', help = 'fisher-intra, LASSO')
    parser.add_argument('-Feature_ratio', action = 'store', type = str, help = 'The ratio for selecting features from the all radiomics features by either backward selection or forward selection)')
    parser.add_argument('-Label', action = 'store', type = str, help = 'The target label name, e.g., \"Histological grade\", \"T stage\", and so on')
    ...
    parser.add_argument('-Kfold', action = 'store', type = str, help = 'The number for Stratified k-fold cross validation')
    args = parser.parse_args()
    return args

def RemoveMissingData(Feature_Table, Label):
    subject_list = list(Feature_Table.keys())
...
def SVM_Classifier(args):
    ...
    Feature_Table = RemoveMissingData(json.load(json_file), args.Label)
    ...
    if args.LeavePout != None:
        ...
        skf = RepeatedStratifiedKFold(n_splits=K, n_repeats=30)
    ...
    if args.LeavePout != None:
        ...
        ss = skf.split(index_arr, index_y)
    ...
    for i, (train_index, test_index) in enumerate(ss):
        ...
        Train_json_path, Test_json_path = splitter(Feature_Table, args.output_folder, train_index, test_index)
        ...
        if args.method == "LASSO":
            csv_path = os.path.join(args.output_folder, args.Label.split()[1] + "_LassoFeatureSelectionResult.csv")
        SelectionResult, N_features = read_SelectionResult(csv_path)
        if N_features <= 30:
            print("*** Apply backward selection for SVM ***")
            acc_tmp, Xtest, ytest, y_inference, score, classifier, select_feature_counter = backward_selection(SelectionResult, N_features, Train_json_path, Test_json_path, args.Label, args.Feature_ratio, K, select_feature_counter)
            ...
            aucs.append(roc_auc)
            y_gt = np.concatenate([y_gt, ytest])
            y_pred = np.concatenate([y_pred, y_inference])
    ...
    print(classification_report(np.array(y_gt), np.array(y_pred), target_names=["LG", "HG"]))
    ...
    std_auc = np.std(aucs)
    ax_roc.plot(mean_fpr, mean_tpr, color='b', label=r'Mean ROC (AUC = %0.2f $\pm$ %0.2f)' % (mean_auc, std_auc), lw=2, alpha=.8)
    std_tpr = np.std(tprs, axis=0)
    tprs_upper = np.minimum(mean_tpr + std_tpr, 1)
    tprs_lower = np.maximum(mean_tpr - std_tpr, 0)
    ax_roc.fill_between(mean_fpr, tprs_lower, tprs_upper, color='grey', alpha=.2, label=r'$\pm$ 1 std. dev.')
    ax_roc.set(xlim=[-0.05, 1.05], ylim=[-0.05, 1.05], title="Receiver Operating Characteristic (ROC) curves")
    ...
    ax_bar.barh(courses, values, height = 1, color ='grey', edgecolor='b') 
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=np.array(['LG', 'HG']))
    disp.plot()
    """
    fpr = dict()
    tpr = dict()
