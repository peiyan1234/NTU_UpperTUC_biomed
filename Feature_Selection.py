...
def get_Weight_matrix(FeatureArray, LabelVector):
...
def IntraFeatureScoring(FeatureArray, LabelVector): 
    #use Fisher Score
    W = get_Weight_matrix(FeatureArray, LabelVector)
    D = np.array(W.sum(axis=1))
    tmp = np.dot(np.transpose(D), FeatureArray)
...
def GetIntraScores_Table(FeatureArray, LabelVector):
    global IntraScores
    scores = IntraFeatureScoring(FeatureArray, LabelVector)
    subj_list = list(Feature_Table.keys())
    filter_list = list(Feature_Table[subj_list[0]]['Features'].keys())
    Original_radiomics_list = list(Feature_Table[subj_list[0]]['Features'][filter_list[0]].keys())
    count = 0
...
def save_IntraScores_Table(LabelName, output_folder):
    GetIntraScores_Table(*GetIntraFeatureArray(LabelName))
    csv_path = os.path.join(output_folder, LabelName.split()[1] + "_IntraFeatureScores.csv")
    ...
    for _key_ in key_list:
        ...
        for _inner_key_ in inner_key_list:
            if _inner_key_ == 'Wavelet':
                ...
                if score1 > score2 and score1 > fisher_score:
                    Select_Table[_key_] = f"{_inner_key_} High Pass"
                    fisher_score = score1
                if score2 > score1 and score2 > fisher_score:
...
def save_IntraFeatureSelectionResult(LabelName, output_folder):
    Select_Table = GetIntraFeatureSelectionResult()
    csv_path = os.path.join(output_folder, LabelName.split()[1] + "_IntraFeatureSelectionResult.csv")
    csv_file = open(csv_path, 'w')
    writer = csv.writer(csv_file, dialect='excel')
        ...
        for _index_ in range(len(radiomics_list)):
            ...
            if "shape2D" in _radiomics_:
                ...
                if _filter_ == 'Wavelet':
                    ...
                    if check_radiomics_H == IntraSelect_Table[_key_] and _radiomics_.split('_')[0] == 'H':
                        if selected_msk[count]:
                            Select_Table[_radiomics_] = [_filter_]
                        count += 1
                    if check_radiomics_L == IntraSelect_Table[_key_] and _radiomics_.split('_')[0] == 'L':
                        if selected_msk[count]:
                            Select_Table[_radiomics_] = [_filter_]
                        count += 1
                else:
            ...
            for _radiomics_ in radiomics_list:
                if "shape2D" in _radiomics_:
                    ...
                    if _filter_ == 'Wavelet':
                        ...
                        check_radiomics_H = f"{_filter_} High Pass"
                        check_radiomics_L = f"{_filter_} Low Pass"
                        if check_radiomics_H == Select_Table[_key_] and _radiomics_.split('_')[0] == 'H':
                            feature_values.append(Feature_Table[_subj_]['Features'][_filter_][_radiomics_])
                        if check_radiomics_L == Select_Table[_key_] and _radiomics_.split('_')[0] == 'L':
                            feature_values.append(Feature_Table[_subj_]['Features'][_filter_][_radiomics_])
                    else:
...
def Fisher_Feature_Selection(args):
    ...
    save_IntraScores_Table(args.Label, args.output_folder)
    save_IntraFeatureSelectionResult(args.Label, args.output_folder)
    InterFeatureReduction(args)

def save_LASSO_FeatureSelectionResult(LabelName, selected_msk, output_folder):
    Select_Table, Feature_Selected_Table = GetFeatureSelectionResult(selected_msk)
    csv_path = os.path.join(output_folder, LabelName.split()[1] + "_LassoFeatureSelectionResult.csv")
    csv_file = open(csv_path, 'w')
    writer = csv.writer(csv_file, dialect='excel')
    headers = ['Feature Name', 'Selected Filter']
    writer.writerow(headers)
    ...
    json_path = os.path.join(output_folder, LabelName.split()[1] + "_LassoFeatureSelectionResult.txt")
    json_file = open(json_path, 'w')
    json_content = json.dumps(Feature_Selected_Table, indent = 4)
...
def LASSO_Feature_Selection(args):
    ...
    save_LASSO_FeatureSelectionResult(args.Label, selected_msk, args.output_folder)

def main():
    parser = argparse.ArgumentParser(
            prog='Radiomics Feature Selection',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            description=API_description)

    parser.add_argument('-Table', action = 'store', type = str, help = 'The absolute path to the DATA TABLE (*.json).')
    parser.add_argument('-Label', action = 'store', type = str, help = 'The target label name, e.g., \"Histological grade\", \"T stage\", and so on')
    #parser.add_argument('-number', action = 'store', type = int, help = 'The number of features will be selected')
    parser.add_argument('-output_folder', action = 'store', help = 'The absolute path to the output folder used to store selected Feature Table')
